...
function renderMainPage() {
    ...
    if (coupleData.startDate) {
        ...
        document.getElementById('ymd-counter').textContent = `${years} năm ${months} tháng ${days} ngày`;
    } else {
...
function renderPartnerCharacter(containerId, partner) {
    ...
    if (hasFeeling) {
        feelingHTML = `
            <div class="text">"${partner.feeling}"</div>
...
function renderEvents(docs) {
    ...
    list.innerHTML = '';
    if (docs.length === 0) {
        list.innerHTML = `<div class="card text-center">Chưa có sự kiện nào được thêm.</div>`;
        return;
    }
    ...
    docs.forEach(eventData => {
        ...
        eventElement.innerHTML = `
            ...
            <div class="flex-grow-1">
                ...
                <p class="event-details"><i class="far fa-clock icon-clock"></i> Lặp lại: ${getRepeatText(event.repeat, event.intervalDays)}</p>
            </div>
            <div class="text-center">
                <div class="event-days-count">${diffDays > 0 ? diffDays : 'Hôm nay'}</div>
...
function renderPartnerSettings(elementId, partner) {
    ...
    if (!partner) {
        container.innerHTML = `<p class="setting-note">Thông tin nửa kia chưa có.</p>`;
        return;
    }
    container.innerHTML = `
        <h4 class="setting-partner-name">${partner.name} (${partner.gender === 'male' ? 'Nam' : 'Nữ'})</h4>
        <div class="space-y-3">
            <div>
                <label class="setting-field-label">Tên</label>
                <input type="text" data-partner-id="${partner.userId}" data-field="name" value="${partner.name}" class="input-field">
            </div>
            <div>
                <label class="setting-field-label">Ngày sinh</label>
                <input type="date" data-partner-id="${partner.userId}" data-field="dob" value="${partner.dob}" class="input-field">
            </div>
             <div>
                <label class="setting-field-label">URL Ảnh đại diện (Nhân vật)</label>
                <input type="text" data-partner-id="${partner.userId}" data-field="photoURL" value="${partner.photoURL}" class="input-field">
            </div>
        </div>
    `;
}
...
toggleFiltersBtn.addEventListener('click', () => {
    ...
    toggleFiltersBtn.querySelector('.filter-chevron').classList.toggle('rotate-180');
});
...
function renderStories(stories) {
    ...
    if (!feed) return;
    if (stories.length === 0) {
        feed.innerHTML = `<div class="card text-center">Không tìm thấy story nào phù hợp.</div>`;
        return;
    }
    ...
    feed.innerHTML = stories.map(story => {
        ...
        const tagsHTML = story.tags ? story.tags.split(',').map(tag => `<span class="tag-bubble">${tag.trim()}</span>`).join(' ') : '';
            ...
            <div class="story-card" id="story-${story.id}">
                ...
                <div class="story-body">
                    <div class="story-author-info">
                        <img src="${partner?.photoURL || 'https://placehold.co/150x150/fbcfe8/ec4899?text=?'}" class="author-avatar">
                        <div>
                            <p class="author-name">${story.authorName}</p>
                            <p class="story-timestamp">${date ? date.toLocaleString('vi-VN') : 'Không có ngày'}</p>
                        </div>
                    </div>
                    ...
                    <div class="story-meta">
                        <div>
                            ${story.location ? `<i class="fas fa-map-marker-alt icon-location"></i> ${story.location}` : ''}
                        </div>
                    ...
                    <div class="story-actions">
                        <div class="like-box">
                         <button class="like-btn ${isLiked ? 'liked' : ''}" data-story-id="${story.id}">
                            ...
                            <span class="like-count">${story.likes?.length || 0}</span>
                        </button>

                        <div class="like-display">
                            <span class="liker-names-display">${likerNames ? ` ${likerNames}` : ''}</span>
                        </div>
                        </div>
...
function renderChatMessages(docs) {
    ...
    chatMessages.innerHTML = docs.map(msg => {
        ...
        const alignClass = isMe ? 'is-me' : 'is-other';
        return `
            <div class="message-container ${alignClass}">
                ...
                <div class="message-bubble-wrapper">
                    ...
                    <div class="message-timestamp ${alignClass}">
...
function updateChatModeVisuals(isSecret) {
    chatHeader.classList.toggle('secret-mode-header', isSecret);
    toggleChatModeBtn.classList.toggle('mode-secret', isSecret);
    toggleChatModeBtn.classList.toggle('mode-normal', !isSecret);
    toggleChatModeBtn.innerHTML = isSecret ? '<i class="fas fa-user-secret icon"></i>Bí mật' : '<i class="fas fa-comments icon"></i>Bình thường';
}
...
function renderStreakLove() {
    ...
    // Reset classes first
    streakHeartContainer.className = 'streak-heart';
    ...
    if (partner1) {
        ...
        icon1.className = streak.p1_checked ? 'fas fa-check-circle status-icon-success' : 'fas fa-times-circle status-icon-fail';
    }
    if (partner2) {
        ...
        icon2.className = streak.p2_checked ? 'fas fa-check-circle status-icon-success' : 'fas fa-times-circle status-icon-fail';
    }
    ...
    if (hasCheckedIn) {
        checkInBtn.innerHTML = `<i class="fas fa-calendar-check icon"></i> Đã điểm danh hôm nay`;
    } else {
        checkInBtn.innerHTML = `<i class="fas fa-calendar-check icon"></i> Điểm danh hôm nay`;
    }
}
