...
if (localStorage.getItem('soNgayDiemDanh')) {
    ...
    document.getElementById('DiemDanh').textContent = soNgayDiemDanh + ' ngày';
  } else {
    // Nếu chưa có dữ liệu trong local storage, gán giá trị mặc định là 0
    var soNgayDiemDanh = 0;
  }

  // Kiểm tra nếu đã điểm danh trong ngày, ẩn nút "Điểm Danh"
  if (localStorage.getItem('ngayDiemDanh') === getCurrentDate()) {
    document.getElementById('diemDanhBtn').style.display = 'none';
  }
  ...
  document.getElementById('xoaDuLieuBtn').addEventListener('click', function() {
    ...
    // Hiển thị nút "Điểm Danh" khi xoá dữ liệu
    document.getElementById('diemDanhBtn').style.display = 'block';
  });
